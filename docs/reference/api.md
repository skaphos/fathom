...
| `path` _string_ | Path is a location on the filesystem whose headroom is measured. Required<br />for DiskHeadroom and InodeHeadroom and rejected for every other type. The<br />operator mounts it into the agent read-only, so it must be a traversal-free<br />absolute path under one of the operator-approved prefixes.<br />The operator uses a non-creating hostPath Directory mount. If Path does<br />not exist on a node, Kubernetes cannot start that node's agent pod;<br />AgentReady becomes False and coverage remains incomplete while the last<br />complete verdict is retained. This avoids mutating the host or measuring<br />the filesystem that would have held a newly created directory. Only a path<br />missing beneath a mounted directory is reported Skipped by the agent. |  | MaxLength: 512 <br />Optional: \{\} <br /> |
