...
- **`<check>-node-agent`** / **`<check>-node-health-agent`** (RoleBinding, per
  check): binds the ClusterRole to the per-check agent ServiceAccount **in the
  check's namespace only** — the ClusterRole's verbs never apply cluster-wide.
  `NodeHealthCheck` reuses the same ClusterRole rather than minting a second
  one: both agents need exactly the same three ConfigMap verbs, and one
  converged singleton is one fewer object to audit.
