...
| fathom.skaphos.io | addonchecks, addondefinitionbindings, addondefinitions, clusterhealths, healthchecks | get, list, watch | The operator's own primary resources. The reconcilers read these objects and write only their `/status` subresource (granted separately below); they never create, update, patch, or delete the objects themselves (AddonChecks and ClusterHealths are authored by users/GitOps, not materialized by the operator). Read-only access plus the status grant is the full set the controllers exercise. **AddonDefinitions and AddonDefinitionBindings** (runtime addon definitions, default-off) are read-only here by design and that is the security property, not an implementation detail: a binding is the administrator's authorization that a definition may borrow a dedicated ServiceAccount, so the operator must never be able to author or amend one. Deliberately **no `create`/`update`/`patch`/`delete` on either kind** — the operator cannot enable a disabled binding, retarget it at another identity, or widen its namespace scope, which is what keeps `enabled=false` a decision only a human or GitOps can reverse. The binding is namespaced to the operator namespace and only that namespace carries authority, so the cluster-wide `list`/`watch` is wider than strictly required; it comes from the shared controller-gen marker rather than from a read the code performs, and the reconciler refuses any binding outside the configured operator namespace. AddonDefinition is cluster-scoped and genuinely requires the ClusterRole. **Not granted:** no Lease access is added for this feature — the drain path's Lease read is namespaced and rides the existing leader-election Role — and no SubjectAccessReview grant, because `requestedReads` is a bounded manifest and diagnostic, never an effective-permission oracle. |
...
| fathom.skaphos.io | addonchecks/status, addondefinitionbindings/status, addondefinitions/status, clusterhealths/status, dnschecks/status, healthchecks/status, nodecertificatechecks/status, nodehealthchecks/status | get, patch, update | Status subresource writes for the operator's own kinds — conditions, observedGeneration, and roll-up state. Status is written through the dedicated subresource so spec and status authority stay separate. For AddonDefinitionBindings that separation is load-bearing rather than stylistic: the operator publishes `Drained`, `Ready`, `activeRuns` and the observed `leaderEpoch` here, and **status never confers authority** — a status write cannot enable a binding, and the reconciler re-reads the spec through an uncached control-plane read before acting on it, so a status write can never be mistaken for a grant. |
...
| networking.k8s.io | networkpolicies | create, get, list, update, watch | The per-check NetworkPolicy for node-agent pods (#153): metrics ingress only from namespaces labeled `metrics: enabled`, and egress limited to destination ports TCP 443 and 6443. The egress rule is deliberately port-only; it does not restrict destination addresses, so an enforcing CNI still permits any destination reachable on either port. It must be authored at runtime because it lives in the check's namespace, which is known only when the check is created. The policy's pod selector is fixed by the controller to the agent labels, so the operator only ever isolates its own pods. No `delete` — the policy is owner-referenced and garbage-collected with the check. |
| rbac.authorization.k8s.io | roles, rolebindings | create, get, list, update, watch | Node-agent provisioning: each check owns a `<service-account>-report-access` Role and RoleBinding in its namespace. The Role grants ConfigMap `create` (which Kubernetes cannot constrain by `resourceNames`) plus `get`/`update` only on canonical report names derived from scheduled, nonterminating pods owned by that check's DaemonSet; an empty fleet leaves the Role at `create` only, and departed names are revoked. Escalation prevention holds because the operator possesses every verb it confers. **No ClusterRole or ClusterRoleBinding grant, no `bind`, no `escalate`, no `delete`** — namespaced objects are owner-referenced. During migration, an owner-controlled legacy node-agent RoleBinding is recognized by its immutable `fathom-node-agent-role` reference and has its subjects cleared; the operator does not change its immutable RoleRef or delete that binding or the old ClusterRole. |
...
reconcile time. The `pods` grant follows probe pods into whatever namespace the
question is being asked about — the addon under check for adapter probes
(ADR-0003), and the check's own namespace for `DNSCheck` (#266). That is
precisely the property that makes the signal representative: a probe answers
from where the workload lives, not from where the operator lives.

For `DNSCheck` it is also a security property rather than only a fidelity one.
Resolving from the check's own namespace makes a check author's reach exactly
equal to their existing reach, so the namespace's own NetworkPolicy governs the
query and no separate allowlist of permitted resolver addresses is needed. The
alternative — resolving from `fathom-system` — would let any author of a check
borrow the operator namespace's egress posture.

**What bounds the blast radius instead:**

- **Verb minimalism.** No `patch`/`delete` on ServiceAccounts, RoleBindings,
  or NetworkPolicies; no ClusterRole grant at all; no
  `create`/`delete` on user checks; no Secret access at all. On pods
  specifically: no `watch` (probe pods are polled, so nothing needs one), no
  `update`/`patch`, and none of the `pods/exec`, `pods/log`, or
  `pods/portforward` subresources.
...
- **`<service-account>-report-access`** (Role and RoleBinding, per check):
  grants that ServiceAccount ConfigMap `create`, plus `get` and `update` only
  on canonical report names for scheduled, nonterminating pods owned by its
  DaemonSet. An empty fleet leaves the Role at `create` only; once pods are
  observed, it adds the named `get`/`update` rule and drops departed names.
  Very long names use a deterministic
  `fathom-report-access-<hash>` fallback so the RBAC object name remains valid.

The old shared `fathom-node-agent-role` ClusterRole remains in clusters for
upgrade compatibility, but the operator no longer reads, updates, creates, or
deletes it, and it creates no legacy `<check>-node-agent` or
`<check>-node-health-agent` RoleBinding. On upgrade, an owner-controlled legacy
binding to that exact ClusterRole has its Subjects cleared while its immutable
RoleRef is left untouched. After every check has migrated and administrators
have verified that no external binding relies on the shared role, the old
ClusterRole may be removed by an administrator.
