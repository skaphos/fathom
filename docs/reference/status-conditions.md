...
| `Paused` | `True / Paused` | `spec.paused=true`; the operator clears the scoped report Role, deletes the agent DaemonSet, and preserves the last status snapshot. Both cleanup operations are attempted independently. | Unset `spec.paused` to recreate the DaemonSet. |
...
| `Ready` | `False / RBACProvisioningFailed` | Per-check ServiceAccount or namespaced report Role/RoleBinding provisioning failed. The Role grants the agent `create` on ConfigMaps plus `get`/`update` only on the names of its current node reports; a legacy owner-referenced binding is drained during migration. The historical verdict is retained. When the failure is during legacy RBAC migration, successful cleanup uses this reason; cleanup failure uses `AgentRevocationFailed`. | Check operator RBAC and admission failures. |
| `Ready` | `False / RBACRevocationFailed` | While pausing, the scoped Role or legacy binding could not be cleared, or the owned DaemonSet could not be deleted. Both revocation steps are attempted independently, so the agent may still be running or may have been removed; agent state and coverage are unknown. | Restore Role/RoleBinding update and DaemonSet delete access, then retry the pause. |
| `Ready` | `False / AgentRevocationFailed` | Admission enforcement or RBAC provisioning failed and cleanup also failed while revoking the previous agent. The message joins the original provisioning error with the cleanup error; the historical verdict is retained and agent state and coverage are unknown. | Restore Role/RoleBinding update and DaemonSet delete access, then retry. Remove the DaemonSet manually if urgent. |
...
| `AgentPrivileged` | `Unknown / AgentRevocationFailed` | The spec was rejected but scoped report access could not be cleared or the previous generation's DaemonSet could not be removed; both revocation steps are attempted independently, so that agent may still be running with its earlier privileges. `Ready` carries the joined cleanup error and the reconcile retries. | Fix Role/RoleBinding update or DaemonSet delete access and API availability; remove the DaemonSet by hand if urgent. |
...
| `AgentPrivileged` | `True / HostNetwork` | A `KubeletHealthz` item put the agent on the host network. The per-check NetworkPolicy does not isolate it; the message names the host liveness/listener port. | Confirm this is intended; drop the item to return to the hardened profile. |
...
| `Ready` | `False / RBACProvisioningFailed` / `AdmissionPolicyProvisioningFailed` / `NetworkPolicyProvisioningFailed` / `DaemonSetProvisioningFailed` | Provisioning failed; the condition is persisted and the historical verdict is retained. Per-check report RBAC uses a namespaced Role granting the agent `create` on ConfigMaps and `get`/`update` only on current report names. During migration, any owned legacy binding to the shared ClusterRole is drained. Early ServiceAccount or legacy-binding RBAC migration failures, and admission-policy failures, attempt access revocation and DaemonSet deletion independently. | As for `NodeCertificateCheck`. |
| `Ready` | `False / AgentRevocationFailed` | Admission enforcement, item validation, or early ServiceAccount/legacy-binding RBAC migration failed and cleanup also failed. Both access revocation and DaemonSet deletion are attempted independently; the message includes the cleanup error and joins the original failure when applicable. The historical verdict remains retained. | Restore Role/RoleBinding update and DaemonSet delete access, then retry; remove the agent manually if urgent. |
