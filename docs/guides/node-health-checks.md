...
| `ContainerRuntime` | A connection to the CRI socket (dial + close; the agent speaks no CRI). | `socketPath` (defaults to `/run/containerd/containerd.sock`; k3s/RKE2 use `/run/k3s/containerd/containerd.sock`, CRI-O `/run/crio/crio.sock`, cri-dockerd `/var/run/cri-dockerd.sock`) | Accepts a connection → Pass; refuses → Fail. A socket that disappears while the agent runs → Fail. A socket absent when the agent *starts* keeps the pod from starting at all (`hostPath` type `Socket`), which shows as `AgentReady=False` and incomplete coverage rather than a verdict — see [Troubleshooting](#troubleshooting). |
