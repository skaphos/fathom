...
// families exercise the Optional-absence contract. Index order: [0]=Deployment,
// [1]=pod, [2]=mutating webhook, [3]=validating webhook, [4..]=CRDs.
func istioHealthyObjects() []clientObject {
	objs := []clientObject{
		...
		validatingConfig("istio-validator-istio-system",
...
func TestIstio_UnpopulatedCABundleFails(t *testing.T) {
	...
	objs := istioHealthyObjects()
	objs[2] = mutatingConfig("istio-sidecar-injector",
		wiredEntry("rev.namespace.sidecar-injector.istio.io", "istio-system", "istiod", nil),
	)
	result, err := NewIstioEngine().Run(context.Background(), adapter.Request{
...
func TestIstio_MissingDeploymentFails(t *testing.T) {
	...
	result, err := NewIstioEngine().Run(context.Background(), adapter.Request{
		Client: newFakeClient(t, istioHealthyObjects()[2:]...),
		Logger: logr.Discard(),
