...
var VpaDefinition = AddonDefinition{
	...
	RBAC: []adapter.PolicyRule{
		{APIGroups: []string{"apps"}, Resources: []string{"deployments"}, Verbs: []string{"get", "list"},
			Justification: "Read the three VPA Deployments (recommender, updater, admission-controller) to score readiness. list because VPA is Optional and may be absent and the names/namespace are policy-overridable; read-only."},
		...
		{APIGroups: []string{"apiextensions.k8s.io"}, Resources: []string{"customresourcedefinitions"}, Verbs: []string{"get", "list"},
			Justification: "Read the VPA CRDs to verify they are Established and serve a supported version. list is needed to check several CRDs; read-only."},
		{APIGroups: []string{"autoscaling.k8s.io"}, Resources: []string{"verticalpodautoscalers"}, Verbs: []string{"get", "list"},
			Justification: "List VerticalPodAutoscaler objects cluster-wide to score their RecommendationProvided condition. Scoped to VerticalPodAutoscalers only — not the Checkpoints, which are read solely as CRDs — and read-only."},
