...
var KedaDefinition = AddonDefinition{
	...
	RBAC: []adapter.PolicyRule{
		{APIGroups: []string{"apps"}, Resources: []string{"deployments"}, Verbs: []string{"get", "list"},
			Justification: "Read the three KEDA Deployments (operator, metrics-apiserver, admission-webhooks) to score readiness. list because KEDA is Optional and may be absent and the names/namespace are policy-overridable (Helm release fullname); read-only."},
		...
		{APIGroups: []string{"apiextensions.k8s.io"}, Resources: []string{"customresourcedefinitions"}, Verbs: []string{"get", "list"},
			Justification: "Read the KEDA CRDs to verify they are Established and serve a supported version. list is needed to check several CRDs; read-only."},
		{APIGroups: []string{"keda.sh"}, Resources: []string{"scaledobjects"}, Verbs: []string{"get", "list"},
			Justification: "List ScaledObjects cluster-wide to score their Ready/Paused conditions. Scoped to the keda.sh group and to ScaledObjects only — not TriggerAuthentications, which can reference trigger credentials — and read-only."},
