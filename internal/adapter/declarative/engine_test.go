...
func TestNewEngine_Validation(t *testing.T) {
	cases := []struct {
		...
		{"field check without list kind", AddonDefinition{AddonType: "x", Families: []FamilyDefinition{{Name: "f", Fields: []FieldCheck{{APIVersion: "example.io/v1", Kind: "Gauge", FieldPath: []string{"status", "phase"}, ExpectedValue: "Steady"}}}}}},
		{"field check without field path", AddonDefinition{AddonType: "x", Families: []FamilyDefinition{{Name: "f", Fields: []FieldCheck{{APIVersion: "example.io/v1", Kind: "Gauge", ListKind: "GaugeList", ExpectedValue: "Steady"}}}}}},
		{"field check without expected value", AddonDefinition{AddonType: "x", Families: []FamilyDefinition{{Name: "f", Fields: []FieldCheck{{APIVersion: "example.io/v1", Kind: "Gauge", ListKind: "GaugeList", FieldPath: []string{"status", "phase"}}}}}}},
