...
type Options struct {
	...
	// the Pod's namespace via the downward API (FATHOM_NAMESPACE) and is required:
	// empty while in-cluster fails closed at startup so adapters cannot silently
	// run as the operator SA (SKA-162). Empty is only valid for local
...
const DefaultProbeImage = "ghcr.io/skaphos/fathom-probe:v0.4.1" // x-release-please-version
...
// DaemonSet unless overridden by --node-agent-image. The version tag is bumped
...
const DefaultNodeAgentImage = "ghcr.io/skaphos/fathom-node-agent:v0.4.1" // x-release-please-version
