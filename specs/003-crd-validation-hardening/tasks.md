...
- [X] T023 [US3] Fixture matrix test (as built: `scripts/crd_compat_gate_test.go`, matching the repo's Go gate-test convention rather than the planned shell test): synthetic old/new CRD pairs + temp allowlist exercising the contract's pass/fail matrix (no change / added optional field / new CRD / removed field / tightened validation / allowlisted / stale entry / malformed allowlist); runnable locally without tags via `go test ./scripts/ -run TestCRDCompat` (depends on T021)
...
- [X] T025 [US3] CI job `crd-compat` in `.github/workflows/ci.yml`: checkout with `fetch-depth: 0` (tags needed), setup-go, run `go -C tools tool task crd-compat`; the fixture gate tests run in the existing Test job as part of the `scripts` package (as-built change from the planned separate shell-test step); pin action SHAs matching existing jobs (depends on T024)
