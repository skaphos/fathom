...
1. **Given** an executable check that last ran some time ago, **When** I run `fathomctl run <kind>/<name>`, **Then** the command returns within the request timeout with an acknowledgement, and the check's next reconcile performs a fresh run rather than waiting for its interval.
...
5. **Given** the same trigger token already consumed, **When** the check next reconciles on its interval, **Then** it does not run again on account of that token.
...
- **FR-005**: Check kinds MUST be addressable as `<kind>/<name>` and as `<kind> <name>`, and kind names MUST be accepted as the Kind, its lowercase form, the plural resource name, and a documented set of CLI-only short aliases. The resource definitions declare no short names, and the documentation MUST say the aliases are not recognised by kubectl.
...
- **FR-007**: Errors MUST state what failed and the next action. Exit codes follow the kubectl convention: `0` on success and `1` on any error; the kind of failure (usage, permission, cluster, timeout) MUST be distinguishable from the message, not the code. FR-021 refines this for `run`.
...
- **FR-009**: `ls` table output MUST show name, verdict, a bounded summary, and last-run age, MUST show next-run where the kind has an interval, and MUST add a namespace column when listing across namespaces (`-A`), left blank for cluster-scoped kinds, matching kubectl.
...
- **FR-015**: `reports` MUST list a check's report history newest-first with timestamp, verdict, summary, and what changed from the prior report; MUST support `--limit` (default 10) and `--since`; and MUST show one report in full with `--report <name>`. Only executable checks write reports: `reports` on a `HealthCheck` MUST follow its reference to the source and say so, and on a `ClusterHealth` MUST fail with the list of sources to query instead.
...
- **FR-019**: `run <kind>/<name>` MUST request an immediate re-evaluation through the existing on-demand trigger contract by writing a fresh, unique trigger token, and MUST return as soon as the request is accepted, printing the token it wrote. The token MUST be the UTC time of the request in RFC 3339 form followed by a short random suffix (for example `2026-09-07T18:04:05Z-7f3a1c`), so that it is unique across concurrent invocations, sorts chronologically, reads as "when was this triggered" in status output, and carries no caller identity. The write MUST identify `fathomctl` as its field manager so the object's managed-fields record which tool wrote the token.
...
- **FR-022**: `run` MUST accept `--all` or a label selector to trigger a set of checks, MUST refuse to run with no target and no selector, and MUST report the outcome per check. `--all` and a selector select executable checks only (`AddonCheck`, `DNSCheck`, `NodeCertificateCheck`) within the namespace scope given by `-n` or `-A`; derived kinds are never selected this way.
- **FR-032**: Before writing any trigger, `run` MUST resolve the full set of executable checks it would touch (including sources reached through a derived kind) and print the count. When that set exceeds 10 checks, `run` MUST ask for interactive confirmation and MUST abort with exit `1` if the answer is not affirmative or if no interactive terminal is available; `--yes` skips the confirmation. `--dry-run` MUST print the resolved set and exit `0` without writing anything.
- **FR-023**: `run` on a paused check MUST fail fast with an explanation and MUST NOT write a trigger. A paused `HealthCheck` is treated the same way: its source is not triggered, because the paused wrapper would not mirror the result.
...
- **Trigger token**: the value carried by the existing on-demand trigger contract: written on the check, it forces a run when it differs from the last consumed token, and the operator records the consumed token in status. The CLI writes a UTC timestamp plus a short random suffix.
...
- **SC-002**: `run --wait` returns the fresh verdict within the check's own configured timeout plus 30 seconds of polling overhead for every executable kind, verified on the Kind e2e cluster. For `NodeCertificateCheck` the bound covers the agent restart on that cluster's node count; larger clusters are expected to pass a longer `--timeout`, as the documentation states.
...
- **SC-008**: The same trigger token never causes more than one run in any real-cluster or automated test.
