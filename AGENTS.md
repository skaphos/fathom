...
- `.claude/skills/speckit-adversarial-review/` and `.specify/extensions.yml`: Skaphos-authored additions, not upstream. They are synced from `skaphos-resources/llm_resources/speckit` (skill and `templates/extensions.yml`); update them there and re-copy rather than editing them here. `extensions.yml` registers the review as a mandatory `after_implement` hook, so `/speckit-implement` runs an independent adversarial review and writes `specs/<NNN>-<short-name>/adversarial-review.md` before it reports completion. This is workflow assistance, not a CI gate.
