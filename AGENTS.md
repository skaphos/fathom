...
- `pkg/addondefinition/`: pure validation, grant planning and offline rendering for typed runtime `AddonDefinition`/`AddonDefinitionBinding` resources. Its built-in inventory file is generated; do not hand-edit it.
...
- `cmd/node-agent/`: the node-agent DaemonSet binary (`Dockerfile.node-agent`); its own dedicated image, never the operator or probe image. `--mode certificates` scans on-disk certificates for `NodeCertificateCheck`; `--mode health` measures filesystem headroom and probes the kubelet and CRI socket for `NodeHealthCheck`.
...
- `internal/adapter/runtime/`: bounded runtime scheduling, budgets, transport and compiled snapshot cache. Runtime check conversion lives in `internal/adapter/declarative/`; dedicated reader and authority clients live in `internal/adapter/impersonation/`. Default-off opt-in loading reserves built-in names before controllers start, uses bounded uncached exact-name GETs for provisional stored UID/generation claims, releases nonconflicting names, and clears matching claims after terminal reconciliation plus direct observation; API errors hold only the affected identity and retries recover.
- `internal/cli/`: the `fathomctl` command tree (`ls`, `describe`, `reports`, `run`, `definition`, `version`), client factory with injectable seams, the kind-descriptor table, and the shared verdict normalisation. Tests are in-package stdlib `testing` over the controller-runtime fake client. Must not import `internal/app`, `internal/adapter`, or `internal/controller`.
- `internal/cli/definition*.go`: offline runtime grant rendering, target-release collision inventory, live UID binding and independent drain verification. These commands never install grants or bindings.
- `internal/controller/`: reconciler implementations (`AddonCheckReconciler`, `HealthCheckReconciler`, `ClusterHealthReconciler`, `NodeCertificateCheckReconciler`, `NodeHealthCheckReconciler`, `DNSCheckReconciler`).
- `internal/controller/addondefinition*_controller.go`: definition and binding lifecycle, eligibility and drain acknowledgement. Runtime work is leader-gated and delegated to a dedicated reader ServiceAccount; status never grants authority.
- `internal/nodecert/`: the on-disk X.509 scan engine and the node-agent↔operator wire contract (no Kubernetes-client deps, so the agent binary stays small). Also hosts the report-authenticity bindings and the kind-qualified report naming both node-scoped kinds share.
- `internal/nodehealth/`: the node-local health engine (`statfs` headroom, kubelet `/healthz`, CRI socket dial) and `NodeHealthCheck`'s wire contract, under the same no-Kubernetes-client discipline. Node conditions are graded by the operator, never here.
...
- `.claude/skills/speckit-*/` and `.agents/skills/speckit-*/`: the spec-kit command skills (`/speckit-specify`, `/speckit-plan`, `/speckit-tasks`, …), vendored from [github/spec-kit](https://github.com/github/spec-kit) once per installed integration (`claude` → `.claude/skills`, `codex` → `.agents/skills`; see `.specify/integration.json`). Regenerate with `specify integration upgrade <agent>` rather than hand-editing — the installer verifies them against the checksums in `.specify/integrations/<agent>.manifest.json`, so a hand-patched skill reports as modified and gets overwritten on the next upgrade. Their upstream copyright is recorded by a scoped override in `REUSE.toml`.
- `speckit-adversarial-review/` (in both skill roots) and `.specify/extensions.yml`: Skaphos-authored additions, not upstream. They are synced from `skaphos-resources/llm_resources/speckit` (skill and `templates/extensions.yml`); update them there and re-copy into every skill root rather than editing them here. `.specify/extensions.yml` is shared by every agent, so a skill it names must exist in every root, or that agent's mandatory hook has nothing to run. `extensions.yml` registers the review as a mandatory `after_implement` hook, so `/speckit-implement` runs an independent adversarial review and writes `specs/<NNN>-<short-name>/adversarial-review.md` before it reports completion. This is workflow assistance, not a CI gate.
...
- `internal/nodecert/*`, `internal/nodehealth/*`, and `cmd/node-agent/*` — the on-disk cert scan and node-health engines and the node-agent DaemonSet binary; they read real host paths, the kubelet, and the CRI socket over hostPath/hostNetwork, which only e2e exercises.
