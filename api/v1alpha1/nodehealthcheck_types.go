...
// +kubebuilder:validation:XValidation:rule="!has(self.path) || (self.path.startsWith('/') && self.path != '/' && !self.path.contains('..') && !self.path.endsWith('/') && !self.path.contains('//') && !self.path.contains('/./') && !self.path.endsWith('/.') && ['/var/lib/kubelet','/var/lib/containerd','/var/lib/docker','/var/lib/etcd','/var/log','/var/lib/rancher','/etc/kubernetes','/run/containerd'].exists(a, self.path == a || self.path.startsWith(a + '/')))",message="path must be an absolute, traversal-free path under an allowed prefix (/var/lib/kubelet, /var/lib/containerd, /var/lib/docker, /var/lib/etcd, /var/log, /var/lib/rancher, /etc/kubernetes, /run/containerd); it must be canonical (no trailing slash, no // or /./ segments)"
// +kubebuilder:validation:XValidation:rule="!has(self.socketPath) || (self.socketPath.endsWith('.sock') && !self.socketPath.contains('..') && !self.socketPath.contains('//') && !self.socketPath.contains('/./') && (['/run/containerd','/var/run/containerd','/run/crio','/var/run/crio','/run/cri-dockerd','/var/run/cri-dockerd','/run/k3s/containerd','/var/run/k3s/containerd'].exists(a, self.socketPath.startsWith(a + '/')) || ['/run/cri-dockerd.sock','/var/run/cri-dockerd.sock'].exists(f, self.socketPath == f)))",message="socketPath must be a traversal-free .sock path under an allowed runtime directory (/run/containerd, /var/run/containerd, /run/crio, /var/run/crio, /run/k3s/containerd, /var/run/k3s/containerd, /run/cri-dockerd, /var/run/cri-dockerd) or the cri-dockerd socket itself (/run/cri-dockerd.sock, /var/run/cri-dockerd.sock); socketPath must be canonical (no // or /./ segments)"
...
type NodeHealthCheckSpec struct {
	...
	// HealthReport (the roll-up transition cadence). Defaults to 5m when
	// unset. Must be at least 10s (MinCheckInterval); the operator clamps
	// stored objects that predate this floor to it at runtime.
	//
	// The runtime cadence is capped: the node-agent re-evaluates at
	// min(interval, 5m), the operator reconciles and refreshes
	// status.lastRunTime on that same cadence (so a silently dead agent is
	// noticed within minutes and a healthy check never reads as stale), and a
	// report counts as fresh for one full agent cycle — that cadence plus
	// three times the capped Timeout. A value above 5m therefore has no
	// further runtime effect; HealthReports are written only when the verdict
...
type NodeHealthCheckStatus struct {
	...
	// evaluation, sorted by node name, as bounded per-node detail. Capped at
	// 100 entries, so in a larger fleet a node can be absent here although it
	// reported; the fleet-wide coverage signal is the CoverageComplete
	// condition, which names the nodes that have not reported. The verdict and
	// coverage are computed across every node before truncation. Like
	// LastResult it is frozen, not cleared, across an incomplete window.
	// +optional
	// +listType=map
	// +listMapKey=node
